...
- (IBAction)publishClicked:(id)sender {
    ...
    [Book addBookToDatabaseWithTitle:self.title author:self.author coverURL:self.coverurl latitude:self.p_bookLat longitude:self.p_bookLon own:self.own sell:self.sell trade:self.trade gift:self.gift userID:[FBSDKAccessToken currentAccessToken].userID withDate:self.date withISBN:self.isbn completion:^(BOOL succeeded, NSError * _Nullable error) {
        if (succeeded) {
            NSLog(@"book posted");
        } else {
            NSLog(@"%@", error);
